...
Object blockToJSON(const CBlock& block, const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("difficulty", getHardness()));
...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "FoxCoin server has now stopped running!";
}


Value getacrecount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getacrecount\n"
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    GenerateFoxcoins(fGenerate, pwalletMain);
    return Value::null;
}


Value getpawspersec(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getpawspersec\n"
...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("TX Fee",            ValueFromAmount(nTransactionFee)));
    obj.push_back(Pair("Minimum Input",     ValueFromAmount(nMinimumInputValue)));
...
Value getdigginginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("Local Paws Per Second:",    getpawspersec(params, false)));
...
Value getnewopening(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "getnewopening [account]\n"
            "Returns a new FoxCoin opening for receiving payments.  "
    ...
    return CFoxcoinAddress(keyID).ToString();
}


CFoxcoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CFoxcoinAddress(account.vchPubKey.GetID());
}

Value getaccountopening(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccountopening <account>\n"
            "Returns the current FoxCoin opening for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <FoxCoin opening> <account>\n"
            "Sets the account associated with the given opening.");

    CFoxcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid FoxCoin opening");
    ...
    // Detect when changing the account of an opening that is the 'unused current key' of another account:
...
Value getopeningsbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getopeningsbyaccount <account>\n"
            "Returns the list of opening for the given account.");

    string strAccount = AccountFromValue(params[0]);

    // Find all openings that have the given account
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CFoxcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CFoxcoinAddress& address = item.first;
...
Value sendtoopening(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoopening <FoxCoin opening> <amount> [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());
    ...
    if (pwalletMain->IsLocked())
        throw JSONRPCError(-13, "Error: Please enter the FoxCoin passphrase with FoxCoinpassphrase first.");
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <FoxCoin opening> <message>\n"
            "Sign a message with the private key of an opening");
    ...
    CFoxcoinAddress addr(strAddress);
    if (!addr.IsValid())
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <FoxCoin opening> <signature> <message>\n"
...
Value getreceivedbyopening(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyopening <FoxCoin opening> [minconf=1]\n"
            "Returns the total amount received by <FoxCoin opening> in transactions with at least [minconf] confirmations.");

    // FoxCoin opening
    CFoxcoinAddress address = CFoxcoinAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid FoxCoin opening");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <to FoxCoin Opening> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());

    string strAccount = AccountFromValue(params[0]);
    CFoxcoinAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid FoxCoin Opening");
    int64 nAmount = AmountFromValue(params[2]);
    int nMinDepth = 1;
...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendmany <fromaccount> {opening:amount,...} [minconf=1] [comment]\n"
    ...
    set<CFoxcoinAddress> setAddress;
    vector<pair<CScript, int64> > vecSend;

    int64 totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CFoxcoinAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid FoxCoin Opening:")+s.name_);

        if (setAddress.count(address))
            throw JSONRPCError(-8, string("Invalid parameter, duplicated opening: ")+s.name_);
        setAddress.insert(address);
...
Value addmultisigopening(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigopening <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            "Add a nrequired-to-sign multisignature opening to the FoxHole\"\n"
            "each key is a FoxCoin Opening or hex-encoded public key\n"
            "If [account] is specified, assign opening to [account].";
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        // Case 1: FoxCoin opening and we have full public key:
        CFoxcoinAddress address(ks);
        if (address.IsValid())
        {
            CKeyID keyID;
    ...
    return CFoxcoinAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CFoxcoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CFoxcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CFoxcoinAddress, tallyitem>::iterator it = mapTally.find(address);
        ...
        {
            Object obj;
            obj.push_back(Pair("opening",       address.ToString()));
...
Value listreceivedbyopening(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            "listreceivedbyopening [minconf=1] [includeempty=false]\n"
            "[minconf] is the minimum number of confirmations before payments are included.\n"
            "[includeempty] whether to include openinges that haven't received any payments.\n"
            "Returns an array of objects containing:\n"
            "  \"opening\" : receiving opening\n"
            "  \"account\" : the account of the receiving opening\n"
            "  \"amount\" : total amount received by the opening\n"
...
Value listreceivedbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "Returns an array of objects containing:\n"
            "  \"account\" : the account of the receiving openinges\n"
            "  \"amount\" : total amount received by openinges with this account\n"
            "  \"confirmations\" : number of confirmations of the most recent transaction included");
...
void ListTransactions(const CWalletTx& wtx, const string& strAccount, int nMinDepth, bool fLong, Array& ret)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& s, listSent)
        {
            ...
            entry.push_back(Pair("opening", CFoxcoinAddress(s.first).ToString()));
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& r, listReceived)
        {
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                entry.push_back(Pair("opening", CFoxcoinAddress(r.first).ToString()));
...
Value listaccounts(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const PAIRTYPE(CTxDestination, string)& entry, pwalletMain->mapAddressBook) {
        if (IsMine(*pwalletMain, entry.first)) // This opening belongs to me
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "FoxHole barricaded; FoxCoin server stopping, restart to run with barricaded FoxHole";
}
...
public:
    ...
    Object operator()(const CScriptID &scriptID) const {
        ...
        BOOST_FOREACH(const CTxDestination& addr, addresses)
            a.push_back(CFoxcoinAddress(addr).ToString());
...
Value validateopening(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateopening <FoxCoin Opening>\n"
            "Return information about <FoxCoin Opening>.");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "FoxCoin server is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "FoxCoin server is downloading acres...");
...
{ //  name                      function                 safe mode?
  //  ------------------------  -----------------------  ----------
    ...
    { "getaccountaddress",      &getaccountopening,      true },
    ...
    { "listreceivedbyaddress",  &listreceivedbyopening,  false },
    { "listreceivedbyopening",  &listreceivedbyopening,  false },
    ...
    { "validateaddress",        &validateopening,        true },
    { "validateopening",        &validateopening,        true },
    { "getbalance",             &getbalance,             false },
    { "move",                   &movecmd,                false },
    { "sendfrom",               &sendfrom,               false },
    { "sendmany",               &sendmany,               false },
    { "addmultisigopening",     &addmultisigopening,     false },
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: foxcoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
//
// JSON-RPC protocol.  FoxCoin speaks version 1.0 for maximum compatibility,
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use foxcoin";
...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if (strMethod == "listreceivedbyopening"  && n > 0) ConvertTo<boost::int64_t>(params[0]);
    if (strMethod == "listreceivedbyopening"  && n > 1) ConvertTo<bool>(params[1]);
    ...
    if (strMethod == "addmultisigopening"     && n > 0) ConvertTo<boost::int64_t>(params[0]);
    if (strMethod == "addmultisigopening"     && n > 1) ConvertTo<Array>(params[1]);
